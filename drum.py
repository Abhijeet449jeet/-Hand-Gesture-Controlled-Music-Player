...
with mp_hands.Hands(min_detection_confidence=0.8, min_tracking_confidence=0.5) as hands:
    while cap.isOpened():
        ...
        print(image.shape[0])
        image = cv2.rectangle(image, (int(image.shape[1] * 0.0), int(image.shape[0] * 0.75)),
                              (int(image.shape[1] * 0.25), int(image.shape[0] * 1)), (255, 0, 0), 4)
        image = cv2.rectangle(image, (int(image.shape[1] * 0.35), int(image.shape[0] * 0.75)),
                              (int(image.shape[1] * 0.60), int(image.shape[0] * 1)), (0, 255, 0), 4)
        image = cv2.rectangle(image, (int(image.shape[1] * 0.75), int(image.shape[0] * 0.75)),
                              (int(image.shape[1] * 1), int(image.shape[0] * 1)), (0, 0, 255), 4)
        ...
        if results.multi_hand_landmarks:
            for num, hand in enumerate(results.multi_hand_landmarks):
                ...
                # Beat
                if hand.landmark[8].y > 0.75 and hand.landmark[8].x > 0 and hand.landmark[
                    8].x < 0.25 and beat_just_played == False:
                    beat.play()
                    beat_just_played = True
                    beat_just_played_hand = num

                if hand.landmark[8].y > 0.75 and hand.landmark[8].x > .35 and hand.landmark[
                    8].x < 0.60 and hihat_closed_just_played == False:
                    hihat_closed.play()
                    hihat_closed_just_played = True
                    hihat_closed_just_played_hand = num

                if hand.landmark[8].y > 0.75 and hand.landmark[8].x > .75 and hand.landmark[
                    8].x < 1 and hihat_opened_just_played == False:
                    hihat_opened.play()
